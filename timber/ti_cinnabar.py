...
def run_cinnabar(df,data=None,col=4,units='uM',title='Amber TI',target_name='',method_name=''):

    if len(list(df.columns))==2:
        name1_col=list(df.columns)[0]
        name2_col=list(df.columns)[1]
        all_ligs=list(set(list(df[name1_col])+list(df[name2_col])))
        all_ligs=[str(x) for x in all_ligs]
        rbfe=True
    elif len(list(df.columns))==1:
        name1_col=list(df.columns)[0]
        all_ligs=list(set(list(df[name1_col])))
        all_ligs=[str(x) for x in all_ligs]
        rbfe=False
    else:
    ...
    if len(all_TI_data)==0:
        print('Error: cannot run cinnabar analysis\n')
        sys.exit()
    ...
    if rbfe:
        ...
        # run cinnabar
        fe = femap.FEMap.from_csv('data_arsenic.csv')
        ...
        DG_plot_shift=output_rbfe_csv(exp_mapping,all_TI_data,fe.to_legacy_graph(),'pairs_amberTI_DDG.csv','compounds_amberTI_DG.csv')

        # remove fe nodes if exp data unknown
        to_remove=[]
        fe_g=fe.to_legacy_graph()
        for x in fe_g.nodes(data=True):
            exp_val=x[1]['exp_DG']
            if exp_val==0.0:
                to_remove.append(x[0])

        for val in to_remove:
            fe_g.remove_node(val)

        if len(fe_g.nodes)>1:
            print('Writing DDG and DG figures\n')

            plotting.plot_DDGs(fe_g,filename='DDGs_fig.png',title=title,target_name=target_name,method_name=method_name,plotly=False)
            plotting.plot_DGs(fe_g,filename='DGs_fig.png',shift=DG_plot_shift,title=title,target_name=target_name,method_name=method_name,guidelines=True,plotly=False)
    ...
    # ABFE 
        ...
        # ABFE DG plot
        if len(exp_mapping)>1 and len(exp_mapping)==len(all_TI_data):
            print('Writing DG figure\n')
            abfe_fe_nx=generate_abfe_fe_nx(all_TI_data,exp_mapping)
            plotting.plot_DGs(abfe_fe_nx,filename='DGs_fig.png',title=title,target_name=target_name,method_name=method_name,guidelines=True,plotly=False,centralizing=False)

    # output statistics
    if check_file('compounds_amberTI_DG.csv') and data and len(exp_mapping)>1:
        compounds=pd.read_csv('compounds_amberTI_DG.csv')
        if len(compounds)>1:
            print('Writing statistics: stats_amberTI.csv\n')
...
def generate_abfe_fe_nx(all_TI_data,exp_mapping):
    ...
    for key,value in all_TI_data.items():
        ...
        if name in exp_mapping:
            calc_DG=np.average(value)
            calc_dDG=np.std(value)
            exp_DG=exp_mapping[name]
            exp_dDG=0.0

            fe.add_node(name, exp_DG=exp_DG, exp_dDG=exp_dDG, calc_DG=calc_DG, calc_dDG=calc_dDG)

    return fe

def fe_statistics(stats_csv,compounds,pairs=None,fe=None):

    df=pd.DataFrame(columns=['N_Lig','N_Pert','R2','KTAU','rho','DG_MUE','DG_RMSE','DDG_MUE','DDG_RMSE'])

    df.at[0,'N_Lig']=len(compounds)
    exp_err=[0 for x in range(0,len(compounds))]

    for statistic in ['R2','KTAU','rho','MUE','RMSE']:
        if fe:
            s=stats.bootstrap_statistic(list(compounds['exp_DG_kcal_mol']),list(compounds['RBFE_calc_DG_kcal_mol']),exp_err,list(compounds['RBFE_calc_dDG_kcal_mol']),statistic=statistic)
        else:
            s=stats.bootstrap_statistic(list(compounds['exp_DG_kcal_mol']),list(compounds['ABFE_calc_DG_kcal_mol']),exp_err,list(compounds['ABFE_calc_dDG_kcal_mol']),statistic=statistic)

        string=f"{s['mle']:.2f} [95% {s['low']:.2f},{s['high']:.2f}]"
        if statistic in ['R2','KTAU','rho']:
    ...
    if fe:
        ...
        for statistic in ['MUE','RMSE']:
             s=stats.bootstrap_statistic(list(pairs['exp_DDG_kcal_mol']),list(pairs['RBFE_calc_DDG_kcal_mol']),exp_err,list(pairs['RBFE_calc_dDDG_kcal_mol']),statistic=statistic)
             string=f"{s['mle']:.2f} [95% {s['low']:.2f},{s['high']:.2f}]"
...
def get_pXC50(dg):
    ...
    try: 
        ic50_uM=revert_free_energy(dg,'uM')
        return -1*np.log10(ic50_uM*1e-6)
    except:
        return None
...
def output_rbfe_csv(exp_mapping,all_TI_data,fe,pairs_csv,compounds_csv):
    ...
    to_remove=[]
    for x in fe_copy.nodes(data=True):
        exp_val=x[1]['exp_DG']
        if exp_val==0.0:
            to_remove.append(x[0])

    for val in to_remove:
        fe_copy.remove_node(val)
    ...
    rbfe_compounds['exp_pXC50']=rbfe_compounds['exp_DG_kcal_mol'].apply(get_pXC50)
    rbfe_compounds['calc_pXC50']=rbfe_compounds['RBFE_calc_DG_kcal_mol'].apply(get_pXC50)
...
def output_abfe_csv(exp_mapping,all_TI_data,compounds_csv):
    ...
    abfe_compounds['exp_pXC50']=abfe_compounds['exp_DG_kcal_mol'].apply(get_pXC50)
    abfe_compounds['calc_pXC50']=abfe_compounds['ABFE_calc_DG_kcal_mol'].apply(get_pXC50)
