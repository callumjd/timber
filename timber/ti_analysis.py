...
def run_analysis(df):
    ...
    for index,row in df.iterrows():
        ...
        # run the analysis
        if criteria_analysis() and len(glob.glob('complex/*_rep*'))>0:
...
def perform_analysis(protocol,ti_repeats):
    ...
    if protocol in ['absolute','absolute-three-step']:
        solv_rest=-1*get_analytical_rest('disang.RST')
    else:
        solv_rest=0.0
    ...
    for rep in range(1,ti_repeats+1):
        ...
        for stage_dir in glob.glob('*/'):
            ...
            complex_free_enegy.update({stage:FE_DVDL(dvdl_list,lambda_list)})
        ...
        for stage_dir in glob.glob('*/'):
            ...
            solvent_free_enegy.update({stage:FE_DVDL(dvdl_list,lambda_list)})
        ...
        # flip signs for absolute
        if protocol in ['absolute','absolute-three-step']:
            for stage,fe_list in complex_free_enegy.items():
                neg_fe_list=list(-1*np.array(fe_list))
                complex_free_enegy.update({stage:neg_fe_list})
            for stage,fe_list in solvent_free_enegy.items():
                neg_fe_list=list(-1*np.array(fe_list))
                solvent_free_enegy.update({stage:neg_fe_list})
...
def FE_DVDL(dvdl_list,lambda_values):
...
def get_dvdl(dir_str):
    ...
    file_list=sorted(file_list, key = lambda x: int(x.strip('/').split('_')[-1]))
    ...
    for mydir in dir_list:
        ...
        for run in runfiles:
            ...
            if len(data)>0 and 'Error' in data[-1]:
                ...
                run_val=int(run.split('.')[-1])-1
                to_resubmit.append(run_val)
            elif len(data)>0 and '(core dumped)' in data[-1]:
                run_val=int(run.split('.')[-1])-1
                to_resubmit.append(run_val)
            elif len(data)>0 and 'symbol lookup error:' in data[-1]:
                run_val=int(run.split('.')[-1])-1
                to_resubmit.append(run_val)
            elif len(data)>0 and 'error while loading shared libraries:' in data[-1]:
                run_val=int(run.split('.')[-1])-1
                to_resubmit.append(run_val)
...
def run_endpoint_mdanalysis(df,prot):
    ...
    for index,row in df.iterrows():
        ...
        if protocol not in ['one-step','three-step']:
            raise Exception('Error: endpoint analysis not implemented for %s protocol!\n' % (protocol))

        # ONE STEP
        if protocol=='one-step':
            os.chdir('complex')
            for rep in range(1,ti_repeats+1):
                os.chdir('one-step_rep'+str(rep)+'/complex_ligands')
                lambda_list=glob.glob('lambda_*')
                lambda_list=sorted(lambda_list, key=lambda x: int(x.split('_')[-1]))

                # LIG analysis
                os.chdir(lambda_list[0])
                if check_file('prod.nc'):
                    print('\nEndpoint MD analysis: %s repeat %d\n' % (name1,rep))
                    os.system('/usr/prog/cadd/amber_tools/timber/versions/0.1/bin/analysis_csv -i prod.nc -p ../../../complex_ligands.prmtop -r %s -lig ":LIG" -sdf ../../../../core/LIG.sdf' % (prot))
                os.chdir('../')

                # MOD analysis
                os.chdir(lambda_list[-1])
                if check_file('prod.nc'):
                    print('\nEndpoint MD analysis: %s repeat %d\n' % (name2,rep))
                    os.system('/usr/prog/cadd/amber_tools/timber/versions/0.1/bin/analysis_csv -i prod.nc -p ../../../complex_ligands.prmtop -r %s -lig ":MOD" -sdf ../../../../sec_lig/MOD.sdf' % (prot))
                os.chdir('../')
        ...
        # THREE STEP
            ...
            os.chdir('complex')
            for rep in range(1,ti_repeats+1):
                os.chdir('three-step_rep'+str(rep))

                # LIG analysis
                os.chdir('complex_decharge/lambda_0')
                if check_file('prod.nc'):
                    print('\nEndpoint MD analysis: %s repeat %d\n' % (name1,rep))
                    os.system('/usr/prog/cadd/amber_tools/timber/versions/0.1/bin/analysis_csv -i prod.nc -p ../../../complex_decharge.prmtop -r %s -lig ":LIG" -sdf ../../../../core/LIG.sdf' % (prot))
                os.chdir('../../')

                # MOD analysis
                lambda_list=glob.glob('complex_recharge/lambda_*')
                lambda_list=sorted(lambda_list, key=lambda x: int(x.split('_')[-1]))
                os.chdir(lambda_list[-1])
                if check_file('prod.nc'):
                    print('\nEndpoint MD analysis: %s repeat %d\n' % (name2,rep))
                    os.system('/usr/prog/cadd/amber_tools/timber/versions/0.1/bin/analysis_csv -i prod.nc -p ../../../complex_recharge.prmtop -r %s -lig ":MOD" -sdf ../../../../sec_lig/MOD.sdf' % (prot))
                os.chdir('../../')
