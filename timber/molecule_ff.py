...
# Just rotatable torsion in rd_mol object
def rotatableTorsions(mol):

    torsionSmarts = '[*!#1]~[*!#1]-[*!#1]~[*!#1]'

    od=OrderedDict()

    all_torsions=mol.GetSubstructMatches(Chem.MolFromSmarts(torsionSmarts))
    for tor in all_torsions:
        bond=mol.GetBondBetweenAtoms(tor[1],tor[2])
        if not bond.IsInRing():
            a=tor[0]
            d=tor[3]
        
            a_degree=int(mol.GetAtomWithIdx(a).GetDegree())
            d_degree=int(mol.GetAtomWithIdx(d).GetDegree())
        
            od.update({tor:int(a_degree+d_degree)})
    
    # put the highest connected atoms first    
    od=dict(sorted(od.items(),key=lambda x: x[1],reverse=True))

    final_torsions=[]
    tracking=[]
    for tor in list(od.keys()):
        ...
        if pair not in tracking:
            tracking.append(pair)
            final_torsions.append(tor)

    return final_torsions
