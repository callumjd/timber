...
def update_mol_coords_pdb(rdmol,pdb_file,residue_name):
    ...
    for i in range(0,len(lig_coords)):
        ...
        y=lig_coords[i].y
        z=lig_coords[i].z
        copy_mol.GetConformer().SetAtomPosition(i,(x,y,z))
...
# add PDB information
def setup_pdb(prmtop,pdb_file,output_prmtop):

    try:
        my_prmtop=pmd.amber.AmberParm(prmtop)

        action=pmd.tools.addPDB(my_prmtop,pdb_file)
        action.execute()
        my_prmtop.write_parm(output_prmtop)
    except:
        print('Warning: it was not possible to append residue info to prmtop\n')
