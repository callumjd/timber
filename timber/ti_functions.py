...
from .openeye_ligprep import openeye_charges
from .openff_ligprep import off_prmtop_converter 
from .md_functions import is_water
...
def run_abfe_setup(rd_mol1,ff='gaff2',dir_1_name='core'):
    ...
    # Openeye assumed present
    charges=openeye_charges('for_parm.sdf','LIG')

    run_antechamber('LIG.sdf',residue_name='LIG',ff=ff)

    LIG=Molecule_ff(name='LIG')
    mol=Chem.SDMolSupplier('LIG.sdf',removeHs=False,sanitize=True)[0]
    LIG=get_rdkit_info(mol,LIG)
    LIG=Info_Mol2('LIG.mol2',LIG,len(mol.GetAtoms()),fields=['name','type'])
    for i in range(0,len(LIG.atoms)):
        LIG.atoms[i].atom_charge=charges[i]
...
def run_rbfe_setup(rd_mol1,rd_mol2,ff='gaff2',dir_1_name='core',dir_2_name='sec_lig',full_mcs=None,align=False):
    ...
    # Openeye assumed present
    charges=openeye_charges('for_parm.sdf','UNL')

    run_antechamber('UNL.sdf',residue_name='UNL',ff=ff)

    LIG=Molecule_ff(name='LIG')
    mol=Chem.SDMolSupplier('UNL.sdf',removeHs=False,sanitize=True)[0]
    LIG=get_rdkit_info(mol,LIG)
    LIG=Info_Mol2('UNL.mol2',LIG,len(mol.GetAtoms()),fields=['name','type'])
    for i in range(0,len(LIG.atoms)):
        LIG.atoms[i].atom_charge=charges[i]
    ...
    MOD=Info_Mol2('UNL.mol2',MOD,len(mol.GetAtoms()),fields=['name','type'])
    for i in range(0,len(MOD.atoms)):
        MOD.atoms[i].atom_charge=charges[i]
...
def write_lambda_windows(media,protocol,schedule,ti_masks,ti_mask_len,hmass,equil_ns,prod_ns,monte_water=0,dir_1_name='core',dir_2_name='sec_lig',nodel=False):
    ...
    for prmtop in prmtop_list:
        ...
        # run_prod file for the cluster
        write_ti_cluster_script(prmtop_path,nodel)
        ...
        # Flag to keep end state prod trajectory files
        os.system('touch lambda_0/KEEPTRAJ')
        os.system('touch lambda_%d/KEEPTRAJ' % (len(lambda_values)-1))
...
def run_prod(df,protocol='one-step',ti_repeats=1,schedule={'complex_ligands':9,'solvent_ligands':9},hmass=True,equil_ns=1,prod_ns=5,monte_water=0,nodel=False):
    ...
    for index,row in df.iterrows():
        ...
        for media in ['complex','solvent']:
            ...
            for rep in range(1+n_offset,ti_repeats+1+n_offset):
                ...
                write_lambda_windows(media,protocol,schedule,ti_masks,ti_mask_len,hmass,equil_ns,prod_ns,monte_water,nodel=nodel)
