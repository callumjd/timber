...
def write_ti_cluster_extend():
    ...
    with open('run_extend.sh','w') as f:
        for line in data:
            if 'AMBERHOME' not in line and 'cd ../' not in line:
                f.write(line)

        if extended==0:
            f.write('$AMBERHOME/bin/pmemd.cuda -O -i prod.in -p $prmtop -c prod.rst7 -ref prod.rst7 -o extend0.out -e extend0.en -inf extend0.info -r extend0.rst7 -x extend0.nc\n')
        else:
            f.write('$AMBERHOME/bin/pmemd.cuda -O -i prod.in -p $prmtop -c extend%d.rst7 -ref extend%d.rst7 -o extend%d.out -e extend%d.en -inf extend%d.info -r extend%d.rst7 -x extend%d.nc\n' % (extended,extended,extended+1,extended+1,extended+1,extended+1,extended+1,extended+1))
...
def write_ti_inputs(protocol,schedule,ti_masks,ti_mask_len,lambda_val,all_lambda_list,hmass=True,equil_ns=1,prod_ns=5,monte_water=0,n_atoms1=0,n_atoms2=0):
    ...
    'DISANG=\"%s\"\n' \
    '\n' \
    % (equil_ns,equil_nstlim,dt,int(1000/(dt*1000)),int(1000/(dt*1000)),int(1000/(dt*1000)),int(5000/(dt*1000)),monte_water,nmc,nmd,nmropt,icfe,lambda_val,int(len(all_lambda_list)),all_lambda,ifsc,charge_mask,ti1,ti2,ti_masks[0],ti_masks[1],disang)
    ...
    'DISANG=\"%s\"\n' \
    '\n' \
    % (prod_ns,prod_nstlim,dt,int(1000/(dt*1000)),int(1000/(dt*1000)),int(1000/(dt*1000)),int(5000/(dt*1000)),monte_water,nmc,nmd,nmropt,icfe,lambda_val,int(len(all_lambda_list)),all_lambda,ifsc,charge_mask,ti1,ti2,ti_masks[0],ti_masks[1],disang)
